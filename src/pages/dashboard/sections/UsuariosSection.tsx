...
export const UsuariosSection: React.FC = () => {
  ...
  useEffect(() => {
    // TODO: Implementar listagem de usuários quando houver endpoint no backend
          ...
          <table className="w-full bg-[var(--color-surface)] rounded-md shadow">
            <thead>
              <tr className="border-b border-[var(--color-border)]">
                ...
                <th className="text-left p-4 text-[var(--color-text-secondary)]">CPF</th>
            ...
            <tbody>
              {usuarios.length === 0 ? (
                ...
                usuarios.map((usuario) => (
                  <tr key={usuario.id} className="border-b border-[var(--color-border)] hover:bg-[var(--color-bg)]">
                    <td className="p-4 text-[var(--color-text)]">{usuario.nome}</td>
                    <td className="p-4 text-[var(--color-text)]">{usuario.email}</td>
                    <td className="p-4 text-[var(--color-text)]">{usuario.cpf || "-"}</td>
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      <div className="flex justify-center gap-2">
                        <button className="p-2 hover:bg-[var(--color-primary-hover)] rounded transition-colors" title="Editar">
                          <FiEdit size={18} />
                        </button>
                        <button className="p-2 hover:bg-[var(--color-primary-hover)] rounded transition-colors" title="Resetar Senha">
                          <FiLock size={18} />
                        </button>
                        <button className="p-2 hover:bg-red-100 dark:hover:bg-red-900 rounded transition-colors text-red-600" title="Excluir">
                          <FiTrash2 size={18} />
                        </button>
                      </div>
                    </td>
